...
export function CertificatesModule() {
  ...
  const typeOptions = useMemo(
    () => [...new Set(items.map((r) => r.type))],
    [items],
  )

  const columns: ColumnDef<CertificateRow>[] = useMemo(
    () => [
      {
        key: 'type',
        header: 'Certificate Type',
        sortable: true,
        filter: { type: 'select', options: typeOptions },
      },
      { key: 'resident', header: 'Resident', sortable: true },
      { key: 'date', header: 'Date Requested', sortable: true, filter: { type: 'date' } },
      {
        key: 'status',
        header: 'Status',
        sortable: true,
  ...
  return (
    <>
      ...
      <DataTable<CertificateRow>
        columns={columns}
        data={items}
        rowKey={(row) => certId(row)}
        searchPlaceholder="Search certificates…"
        emptyMessage='No certificates found. Click "Issue Certificate" to get started.'
        renderActions={(row) => {
          const id = certId(row)
